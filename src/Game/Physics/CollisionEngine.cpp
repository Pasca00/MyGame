...
void CollisionEngine::checkCollision(Player* p, View* v) {
	...
	if (p->getDirection() == DIRECTION_LEFT) {
		if (rect->x + p->getDirection() * p->getVelocity() < v->dstrect.x + v->dstrect.w
			&& rect->x + p->getDirection() * p->getVelocity() >= v->dstrect.x) {
		...
		if (rect->x + rect->w + p->getVelocity() >= v->dstrect.x
			&& rect->x + rect->w + p->getVelocity() < v->dstrect.x + v->dstrect.w) {
